...
def party_creater(country):
    ...
    new_in_power_com = np.intersect1d(G.nodes(), in_power_com).tolist() # ensure it is a list
...
def party_creater_over_world(country):
    ...
    new_in_power_com = np.intersect1d(G.nodes(), in_power_com).tolist()
...
#######
## On the SNAP datasets with build-in comminities
######

def get_snap_largest_com(dataset):
    largest_cm_len = 0
    largest_cm = []
    with open("../data/" + dataset + "/" + "com-" + dataset + ".top5000.cmty.txt") as cmList:
        for line in cmList:
            tmp_cm = line.strip("\n").split("\t") 
            if len(tmp_cm) > largest_cm_len:
                largest_cm_len = len(tmp_cm)
                largest_cm = tmp_cm 
    return [int(i) for i in largest_cm] 
...
def construct_snap_graph(dataset):
    largest_cm = get_snap_largest_com(dataset)
    G = read_snap_graph(dataset)

    return G, largest_cm, G.number_of_nodes()
